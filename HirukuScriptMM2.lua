...
-- НОВЫЕ ИКОНКИ (по вашим ID)
local iconAssets = {
    ["Combat"] = "rbxassetid://1301598910",
    ["Visuals"] = "rbxassetid://1301597837",
    ["Movement"] = "rbxassetid://6031098384",
    ["Misc"] = "rbxassetid://6172185981"
}
...
for i, name in ipairs(Sections) do
    ...
    icon.Image = iconAssets[name]
    icon.ImageColor3 = Color3.fromRGB(150, 150, 150)
    icon.Parent = btn

    local textLabel = Instance.new("TextLabel")
    textLabel.Size = UDim2.new(1, -25, 1, 0)
    textLabel.Position = UDim2.new(0, 25, 0, 14)
...
local function CreateToggle(parent, label, path)
    ...
    toggle.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            elseif path[1] == "SexAura" and path[2] == "Enabled" then
                if Config.SexAura.Enabled then EnableSexAura() else DisableSexAura() end
            elseif path[1] == "SpinBot" and path[2] == "Enabled" then
                if Config.SpinBot.Enabled then EnableSpinBot() else DisableSpinBot() end
            elseif path[1] == "AutoSwing" and path[2] == "Enabled" then
                if Config.AutoSwing.Enabled then EnableAutoSwing() else DisableAutoSwing() end
            end
        end
    end)
end
...
espPanel.Position = UDim2.new(0, 5, 0, 140)
...
chamsPanel.Position = UDim2.new(0, 5, 0, 340)
...
sexAuraPanel.Size = UDim2.new(1, -10, 0, 60)
sexAuraPanel.Position = UDim2.new(0, 5, 0, 850)

local spinBotPanel = CreateSettingPanel(MiscTab, "Spin Bot")
CreateToggle(spinBotPanel, "Enable", {"SpinBot","Enabled"})
spinBotPanel.Size = UDim2.new(1, -10, 0, 60)
spinBotPanel.Position = UDim2.new(0, 5, 0, 920)

local autoSwingPanel = CreateSettingPanel(MiscTab, "Auto Swing")
CreateToggle(autoSwingPanel, "Enable", {"AutoSwing","Enabled"})
autoSwingPanel.Size = UDim2.new(1, -10, 0, 60)
autoSwingPanel.Position = UDim2.new(0, 5, 0, 990)
...
RunService.RenderStepped:Connect(function(dt)
        ...
        if triggerDelay >= Config.Trigger.Delay then
            ...
            if target then
                ...
                if hrp then
                    ...
                    if dist <= Config.Trigger.Range then
                        AttemptAttack()
                    end
                end
            end
    ...
    -- Sex Aura
    if Config.SexAura.Enabled and LocalPlayer.Character then
        ...
        if target then
            ...
            if hrp and targetHrp then
                ...
                if dist > 5 then
                    hrp.CFrame = CFrame.new(myPos + dir * 2)
                else
                    hrp.CFrame = CFrame.new(myPos - dir * 2)
                end
    ...
    -- Auto Swing
    if Config.AutoSwing.Enabled then
        local target = GetClosestTargetByDistance()
        if target and target.Character then
            local hrp = target.Character:FindFirstChild("HumanoidRootPart")
            if hrp and (hrp.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude < 10 then
...
RunService.Heartbeat:Connect(function()
    if Config.ShowCircle.Enabled and LocalPlayer.Character then
        ...
        if humanoid then
            ...
            LastFloorMat = currentFloor
        end
    end
end)

-- Sex Aura
function EnableSexAura()
    SexAuraActive = true
end

function DisableSexAura()
    SexAuraActive = false
end
...
-- Auto Swing
function EnableAutoSwing()
    AutoSwingActive = true
end

function DisableAutoSwing()
    AutoSwingActive = false
end

-- Sky Color (Исправлено)
...
-- Плавное перетаскивание меню (input.Delta - идеально)
...
UserInputService.TouchMoved:Connect(function(input)
    if isDraggingMenu then
        MainFrame.Position = UDim2.new(0, MainFrame.Position.X.Offset + input.Delta.X, 0, MainFrame.Position.Y.Offset + input.Delta.Y)
    end
end)
