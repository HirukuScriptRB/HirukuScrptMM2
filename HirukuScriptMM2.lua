...
local S = {
    ...
    Crosshair=false, Fullbright=false, NoFog=false, Tracers=false, Hitboxes=false, HealthBars=false, AmbientPulse=false,
    ...
    AntiAFK=false,
    Theme="Obsidian", Font="Bangers", Language="English", UIScale=1, MenuOpacity=.90,
...
local THEMES={
    ...
    Ink={panel=Color3.fromRGB(8,8,10),card=Color3.fromRGB(20,20,24),card2=Color3.fromRGB(30,30,36),text=Color3.fromRGB(250,250,255),sub=Color3.fromRGB(145,145,155),accent=Color3.fromRGB(255,255,255),line=Color3.fromRGB(70,70,78),on=Color3.fromRGB(245,245,250),off=Color3.fromRGB(48,48,55)},
    Paint={panel=Color3.fromRGB(27,14,25),card=Color3.fromRGB(42,22,38),card2=Color3.fromRGB(57,29,50),text=Color3.fromRGB(255,248,253),sub=Color3.fromRGB(195,166,188),accent=Color3.fromRGB(255,122,210),line=Color3.fromRGB(91,47,80),on=Color3.fromRGB(255,145,220),off=Color3.fromRGB(74,42,66)},
    Prism={panel=Color3.fromRGB(12,12,20),card=Color3.fromRGB(23,22,35),card2=Color3.fromRGB(34,32,49),text=Color3.fromRGB(248,248,255),sub=Color3.fromRGB(166,164,190),accent=Color3.fromRGB(190,170,255),line=Color3.fromRGB(62,57,91),on=Color3.fromRGB(207,190,255),off=Color3.fromRGB(56,52,72)},
}
...
local CONNS, INST, CHAMS, LABELS, GUNESP, TRACERS, BOXES, DISTANCES, HEALTHBARS, HITBOXES = {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
...
local function openDrawer(name,builder)
    ...
    drawer.Visible=true
    builder()
    local wantedH=math.clamp(drawerY+62,140,math.min(420,Camera.ViewportSize.Y-24))
...
conn(drawer.InputBegan:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.Touch or i.UserInputType==Enum.UserInputType.MouseButton1 then
        -- Only the drawer surface itself starts a drag; child controls keep their own taps.
        if i.Target==drawer then drawerMoving=true; drawerStart=i.Position; drawerOrigin=drawer.Position end
    end
end))
...
local function centerMenu() local sz=menuSize(); local vp=Camera.ViewportSize; main.Size=sz; task.defer(function() main.Position=UDim2.fromOffset(math.max(8,(vp.X-main.AbsoluteSize.X)/2),math.max(8,(vp.Y-main.AbsoluteSize.Y)/2)) end) end
...
local function toggle(parent,name,desc,get,set,settings)
    ...
    txt(c,name,UDim2.new(1,-140,0,22),UDim2.fromOffset(14,7),13,C().text,725)
    txt(c,desc,UDim2.new(1,-140,0,19),UDim2.fromOffset(14,34),9,C().sub,725)
    local sw=btn(c,"",UDim2.fromOffset(44,24),UDim2.new(1,-58,.5,-12),730); sw.BackgroundColor3=C().off
    local dot=fr(sw,UDim2.fromOffset(16,16),UDim2.fromOffset(4,4),Color3.fromRGB(180,180,185),0,731); corner(dot,8)
    local function refresh()
        local on=get(); sw.BackgroundColor3=on and C().on or C().off
        dot.BackgroundColor3=on and C().panel or Color3.fromRGB(180,180,185)
        tw(dot,.14,{Position=on and UDim2.new(1,-20,.5,-8) or UDim2.fromOffset(4,4)})
    end
    local changing=false
    local function flip() if changing then return end; changing=true; set(not get()); refresh(); task.defer(function() changing=false end) end
    conn(sw.Activated:Connect(flip))
    -- Large touch target makes every feature easy to activate on Android while leaving SET free.
    local hit=Instance.new("TextButton"); hit.BackgroundTransparency=1; hit.Text=""; hit.AutoButtonColor=false; hit.Size=UDim2.new(1,-112,1,0); hit.Position=UDim2.fromOffset(0,0); hit.ZIndex=728; hit.Parent=c
    conn(hit.Activated:Connect(flip))
    refresh()
    if settings then
        local sb=btn(c,"SET",UDim2.fromOffset(38,22),UDim2.new(1,-102,.5,-11),729); sb.TextSize=9; sb.BackgroundTransparency=.28
        conn(sb.Activated:Connect(settings))
    end
...
toggle(pages.Visuals,"Health Bars","Show a compact health value above players",function() return S.HealthBars end,function(v) S.HealthBars=v end)
toggle(pages.Visuals,"Hitboxes","Outline character bounds",function() return S.Hitboxes end,function(v) S.Hitboxes=v end)
toggle(pages.Visuals,"Ambient Pulse","Soft animated local ambience",function() return S.AmbientPulse end,function(v) S.AmbientPulse=v end)
...
toggle(pages.Misc,"Spin","Rotate character",function() return S.Spin end,function(v) S.Spin=v end,function() slider(drawerBody,"Spin speed",100,20000,function() return S.SpinSpeed end,function(v) S.SpinSpeed=v end) end)
...
optionList(pages.Settings,"LANGUAGE",{{"English","English"},{"Russian","Russian"}},function() return S.Language end,function(v) S.Language=v end)
optionList(pages.Settings,"FONT",{{"Bangers","Bangers"},{"Gotham Bold","GothamBold"},{"Gotham","Gotham"},{"Source Sans","SourceSans"},{"Arial","Arial"},{"Sci-Fi","SciFi"}},function() return S.Font end,function(v) S.Font=v end)
optionList(pages.Settings,"THEME",{{"Obsidian","Obsidian"},{"Violet","Violet"},{"Midnight","Midnight"},{"Crimson","Crimson"},{"Aurora","Aurora"},{"Ink","Ink"},{"Paint","Paint"},{"Prism","Prism"}},function() return S.Theme end,function(v) S.Theme=v end)
slider(pages.Settings,"Interface Size",80,120,function() return S.UIScale*100 end,function(v)
    S.UIScale=v/100
    if mainScale then mainScale.Scale=S.UIScale end
    task.defer(centerMenu)
end)
slider(pages.Settings,"Menu Opacity",50,95,function() return S.MenuOpacity*100 end,function(v) S.MenuOpacity=v/100; applyStyle() end)
...
local function clearPlayerESP(p)
    for _,tab in ipairs({CHAMS,LABELS,TRACERS,BOXES,DISTANCES,HEALTHBARS,HITBOXES}) do if tab[p] then pcall(function() tab[p]:Destroy() end); tab[p]=nil end end
end
...
local function isPlayerCharacterObject(o)
...
-- MM2 gun-drop detection: only a physical drop on the map is eligible.
-- A Gun/Pistol/GunDrop that is inside a player's Character/Backpack is ignored.
local function droppedGunPart()
    ...
    for _,o in ipairs(Workspace:GetDescendants()) do
        ...
        local validName=(n=="gundrop" or n=="droppedgun" or n=="droppedpistol" or n=="pistoldrop")
        if validName and not isPlayerCharacterObject(o) then
            local host=o:IsA("Model") and o or o:FindFirstAncestorOfClass("Model") or o
            local part=o:IsA("BasePart") and o or (host:IsA("Model") and (host.PrimaryPart or host:FindFirstChild("Handle") or host:FindFirstChildWhichIsA("BasePart",true)))
            if part and part:IsDescendantOf(Workspace) then
                -- A real floor drop has nearby geometry below it. This prevents a held tool
                -- or an object attached high in a character from being selected.
                local hit=Workspace:Raycast(part.Position,Vector3.new(0,-10,0),params)
                local floorOK=hit and (part.Position.Y-hit.Position.Y)<=8
                local inTool=part:FindFirstAncestorOfClass("Tool")
                if floorOK and not inTool then
                    local d=(part.Position-rr.Position).Magnitude
...
local function nearestCoin()
    ...
    local best,dist=nil,math.huge
    for _,o in ipairs(Workspace:GetDescendants()) do
        if o.Name:lower()=="coin" and not isPlayerCharacterObject(o) then
            local p=o:IsA("BasePart") and o or (o:IsA("Model") and (o.PrimaryPart or o:FindFirstChild("Handle") or o:FindFirstChildWhichIsA("BasePart",true)))
...
conn(RunService.RenderStepped:Connect(function(dt)
    ...
    if S.BunnyHop and not S.Fly and not S.CoinFarm then
        local h=hum(char()); if h and h.Health>0 then remember(); local moving=h.MoveDirection.Magnitude>.08
            if moving then
                S.BunnyCurrent=math.min(S.BunnyMax,S.BunnyCurrent+S.BunnyAccel*dt*10)
                h.WalkSpeed=S.BunnyCurrent
                if h.FloorMaterial~=Enum.Material.Air then h.Jump=true end
            else
                S.BunnyCurrent=saved.walk or 16; h.WalkSpeed=saved.walk or 16
            end
        end
    ...
    setNoClip(S.NoClip or S.CoinFarm)
    ...
    if S.CoinFarm and cachedCoin and os.clock()-lastCoin>=S.CoinDelay then
        lastCoin=os.clock(); local r=root(char()); if r and cachedCoin.Parent then
            -- Coin routes are intentionally obstacle-independent: briefly disable collisions and
            -- place the character at the coin so MM2's touch collector can register it.
            setNoClip(true)
            r.CFrame=CFrame.new(cachedCoin.Position+Vector3.new(0,1.15,0))
            pickupPart(cachedCoin)
            if cachedCoin.Parent then pickupPart(cachedCoin) end
        end
    end
    if S.AutoPickupGun and cachedGun and os.clock()-lastGun>=S.AutoPickupDelay then
        lastGun=os.clock(); local r=root(char()); if r and cachedGun.Parent and not isPlayerCharacterObject(cachedGun) then
            local host=cachedGun:FindFirstAncestorOfClass("Model") or cachedGun
            if not host:FindFirstAncestorOfClass("Tool") then
                local old=r.CFrame
                r.CFrame=CFrame.new(cachedGun.Position+Vector3.new(0,1,0))
                pickupPart(cachedGun)
                if cachedGun.Parent and not isPlayerCharacterObject(cachedGun) then pickupPart(cachedGun) end
                task.defer(function() if r.Parent and S.AutoPickupGun then r.CFrame=old end end)
            end
        end
    end
...
conn(RunService.Heartbeat:Connect(function(dt)
    ...
    if visClock>=.25 then visClock=0; if S.Chams or S.RoleLabels then refreshESP() else for p in pairs(CHAMS) do clearPlayerESP(p) end end
        ...
        if S.HealthBars or S.Hitboxes then
            for _,p in ipairs(Players:GetPlayers()) do
                if p~=LP and canTarget(p) then
                    local model=p.Character; local head=model and model:FindFirstChild("Head")
                    if S.HealthBars and head then
                        local bb=HEALTHBARS[p]
                        if not bb then
                            bb=Instance.new("BillboardGui"); bb.Name="HirukuHealth"; bb.Size=UDim2.fromOffset(100,18); bb.StudsOffset=Vector3.new(0,3.8,0); bb.AlwaysOnTop=true; bb.Adornee=head; bb.Parent=gui; HEALTHBARS[p]=bb
                            local t=txt(bb,"",UDim2.fromScale(1,1),UDim2.fromScale(0,0),9,C().text,1130,Enum.TextXAlignment.Center)
                        end
                        local h=hum(model); local t=bb:FindFirstChildWhichIsA("TextLabel",true); if h and t then t.Text=string.format("HP %d",math.max(0,math.floor(h.Health))) end
                    elseif HEALTHBARS[p] then HEALTHBARS[p]:Destroy(); HEALTHBARS[p]=nil end
                    if S.Hitboxes and model then
                        local sb=HITBOXES[p]
                        if not sb then sb=Instance.new("SelectionBox"); sb.Name="HirukuHitbox"; sb.Adornee=model; sb.LineThickness=.02; sb.SurfaceTransparency=1; sb.Color3=ROLECOLOR[role(p)] or ROLECOLOR.Unknown; sb.Parent=gui; HITBOXES[p]=sb end
                        sb.Color3=ROLECOLOR[role(p)] or ROLECOLOR.Unknown
                    elseif HITBOXES[p] then HITBOXES[p]:Destroy(); HITBOXES[p]=nil end
                end
            end
        else
            for p,b in pairs(HEALTHBARS) do pcall(function() b:Destroy() end); HEALTHBARS[p]=nil end
            for p,b in pairs(HITBOXES) do pcall(function() b:Destroy() end); HITBOXES[p]=nil end
        end
        if S.AmbientPulse then
            local pulse=(math.sin(os.clock()*1.2)+1)*.5
            Lighting.Ambient=C().accent:Lerp(C().panel,pulse*.35)
        end
...
function S.Cleanup()
    ...
    for p in pairs(HEALTHBARS) do pcall(function() HEALTHBARS[p]:Destroy() end) end; for p in pairs(HITBOXES) do pcall(function() HITBOXES[p]:Destroy() end) end
    table.clear(CHAMS); table.clear(LABELS); table.clear(TRACERS); table.clear(BOXES); table.clear(DISTANCES); table.clear(GUNESP); table.clear(HEALTHBARS); table.clear(HITBOXES)
    Lighting.Brightness=savedLighting.Brightness; Lighting.Ambient=savedLighting.Ambient; Lighting.OutdoorAmbient=savedLighting.OutdoorAmbient
