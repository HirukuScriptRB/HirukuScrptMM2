...
for i, name in ipairs(Sections) do
    ...
    icon.Position = UDim2.new(0.5, -10, 0, 2)
...
local function SwitchTab(name)
    ...
    -- Исправление скролла: вычисляем высоту контента и ставим край
    task.wait(0.1)
                    ...
                    if hrp and head then
                        ...
                        if onScreen and headOnScreen and footOnScreen then
                            ...
                            if Config.ESP.Box then
                                ...
                                local width = 0.85 * scale -- Хитбокс пошире
...
-- Auto Farm (Исправлено, без задержки)
function IsCoin(obj)
        ...
        if hrp and humanoid then
            ...
            if AutoFarmTarget then
                ...
                if distance <= step or distance < 3 then
                    -- Добавляем монету в собранные, удаляем её линию и сразу летим к следующей
...
function ApplySkyColor()
    ...
    if sky.Parent then sky.Parent = nil end
...
-- Drag & Drop (Возвращено, за любую область кроме кнопок)
