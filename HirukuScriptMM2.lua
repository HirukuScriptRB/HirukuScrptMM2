--// HIRUKU MM2 V20
--// Restored full V18 feature set; V20 is a compatibility/interaction patch, not a rewrite.
...
-- Android/executor-safe GUI parent: prefer gethui only when it is a real callable function.
...
-- The settings drawer is independently draggable on touch so moving it never rotates the camera.
...
drawerDragArea.Size = UDim2.new(1,-58,0,50)
drawerDragArea.Position = UDim2.fromOffset(8,4)
drawerDragArea.ZIndex = 725
drawerDragArea.Active = true
drawerDragArea.Selectable = false
drawerDragArea.Parent = drawer

local drawerDrag = {active=false,input=nil,start=nil,origin=nil}
conn(drawerDragArea.InputBegan:Connect(function(input)
    if input.UserInputType ~= Enum.UserInputType.Touch and input.UserInputType ~= Enum.UserInputType.MouseButton1 then return end
    drawerDrag.active = true
    drawerDrag.input = input
    drawerDrag.start = input.Position
    drawerDrag.origin = drawer.Position
end))
...
conn(UIS.InputChanged:Connect(function(input)
    if not drawerDrag.active or not drawerDrag.start or not drawerDrag.origin then return end
    if input.UserInputType ~= Enum.UserInputType.Touch and input.UserInputType ~= Enum.UserInputType.MouseMovement then return end
    local delta = input.Position - drawerDrag.start
    local vp = Camera.ViewportSize
    local w,h = drawer.AbsoluteSize.X, drawer.AbsoluteSize.Y
    local x = math.clamp(drawerDrag.origin.X.Offset + delta.X, 8, math.max(8,vp.X-w-8))
    local y = math.clamp(drawerDrag.origin.Y.Offset + delta.Y, 8, math.max(8,vp.Y-h-8))
    drawer.Position = UDim2.fromOffset(x,y)
end))
conn(UIS.InputEnded:Connect(function(input)
    if input == drawerDrag.input or input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
        drawerDrag.active=false
        drawerDrag.input=nil
        drawerDrag.start=nil
        drawerDrag.origin=nil
    end
end))
...
local function findNamedNearest(name)
    ...
    for _,d in ipairs(Workspace:GetDescendants()) do
        if d.Name:lower()==wanted and not (myChar and d:IsDescendantOf(myChar)) and not d:FindFirstAncestorOfClass("Tool") then
            local p
...
local function findDroppedGun()
    ...
    for _,d in ipairs(Workspace:GetDescendants()) do
        ...
        local named = n=="gundrop" or n=="droppedgun" or n=="droppedpistol" or n=="pistol" or n=="gun"
        if named then
            -- A Tool/part belonging to any player's Character is a held weapon, not a floor drop.
            local ancestorModel=d:FindFirstAncestorOfClass("Model")
            local owner=ancestorModel and Players:GetPlayerFromCharacter(ancestorModel)
            local insideMyChar=myChar and d:IsDescendantOf(myChar)
            local insideTool=d:FindFirstAncestorOfClass("Tool") ~= nil
            if not owner and not insideMyChar and not insideTool then
                local p
                if d:IsA("BasePart") then p=d
                elseif d:IsA("Model") then p=d.PrimaryPart or d:FindFirstChildWhichIsA("BasePart",true) end
