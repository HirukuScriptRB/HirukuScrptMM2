...
local Config = {
    ...
    ESP = {Enabled = false, Box = true, Name = true, Health = true, Distance = true, Thickness = 1},
...
local iconAssets = {
    ...
    ["Visuals"] = "rbxassetid://6035047377",
    ["Movement"] = "rbxassetid://6034627827",
    ["Misc"] = "rbxassetid://6034630242"
}
...
local function CreateToggle(parent, label, path)
    ...
    toggle.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            elseif path[1] == "InfiniteJump" and path[2] == "Enabled" then
                -- просто храним настройку
            elseif path[1] == "ItemESP" and path[2] == "Enabled" then
                if Config.ItemESP.Enabled then EnableItemESP() else DisableItemESP() end
            end
        end
    end)
end
...
local function CreateSlider(parent, label, path, min, max, decimal)
    ...
    slider.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            UpdateSlider()
            if path[1] == "ESP" and path[2] == "Thickness" then UpdateESPSettings() end
...
CreateSlider(espPanel, "Thickness", {"ESP","Thickness"}, 1, 5, false)
...
chamsPanel.Position = UDim2.new(0, 5, 0, 400)
...
local infiniteJumpPanel = CreateSettingPanel(MiscTab, "Infinite Jump")
CreateToggle(infiniteJumpPanel, "Enable", {"InfiniteJump","Enabled"})
infiniteJumpPanel.Size = UDim2.new(1, -10, 0, 60)
infiniteJumpPanel.Position = UDim2.new(0, 5, 0, 850)

local itemEspPanel = CreateSettingPanel(MiscTab, "Item ESP")
CreateToggle(itemEspPanel, "Enable", {"ItemESP","Enabled"})
itemEspPanel.Size = UDim2.new(1, -10, 0, 60)
itemEspPanel.Position = UDim2.new(0, 5, 0, 920)
...
local function UpdateESPSettings()
    if ESPActive then
        for _, data in pairs(ESPCache) do
            if data and data[1] then data[1].Thickness = Config.ESP.Thickness end
        end
    end
end
    ...
    if AutoFarmActive and LocalPlayer.Character then
        ...
        if hrp and humanoid then
            ...
            if AutoFarmTarget then
                ...
                if distance <= step or distance < 3 then
...
function EnableItemESP()
    for _, obj in ipairs(workspace:GetDescendants()) do
        if IsCoin(obj) then
            local box = Drawing.new("Square")
            box.Size = Vector2.new(4, 4)
            box.Color = Color3.fromRGB(255, 255, 0)
            box.Thickness = 1
            box.Filled = false
            box.Visible = true
            ItemESPCache[obj] = box
        end
    end
end

function DisableItemESP()
    for _, box in pairs(ItemESPCache) do
        box:Remove()
    end
    ItemESPCache = {}
end
...
RunService.RenderStepped:Connect(function()
    if Config.ItemESP.Enabled then
        for obj, box in pairs(ItemESPCache) do
            if obj and obj.Parent then
                local pos, onScreen = Camera:WorldToViewportPoint(obj.Position)
                if onScreen then
...
-- Исправленное перетаскивание меню (без рывков)
...
MainFrame.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
        ...
        if not isClickable and not isDraggingMenu then
            StartDragging(input)
        end
    end
end)
...
UserInputService.TouchMoved:Connect(function(input)
    if isDraggingMenu then
        MainFrame.Position = UDim2.new(
            0, dragStartFramePos.X.Offset + (input.Position.X - dragStartPos.X),
            0, dragStartFramePos.Y.Offset + (input.Position.Y - dragStartPos.Y)
        )
    end
end)
